...
import { FormCustomProps, FormProps } from "./CustomProps";

export function useForm(initialValue: FormProps) {
    const arrErrorMsg = [
    ...
    const addForm = async (form: FormProps) => {
        try {
    ...
    const addFormWithMock = async (form: FormProps) => {
        fetch('https://64e75fafb0fd9648b78fdde6.mockapi.io/formSuggestion', {
            ...
            // handle error
        }).then(messages => { //messages = 'send success'
    ...
    async function handelForm(form: FormProps) {
        let isError = false;
...
export function Form(props: FormCustomProps) {
